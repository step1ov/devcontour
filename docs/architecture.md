...
1. Ведущий агент получает workspace и запускает `start`. Сервер показывает продуктовый процесс до появления конфигурации. После утверждения пользователем продукта, затем архитектуры и стека с C1/C2, `setup` переносит стартовые файлы; `workspace-init` собирает многорепозиторную конфигурацию. Исполнение подключается на том же URL.
