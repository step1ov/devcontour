...
Shell ревьюера всегда исполняется в песочнице ОС: у codex — `--sandbox read-only`, у claude — sandbox Claude Code без сети, с запретом записи в проверяемый каталог, без выхода из песочницы и с отказом запуска, если механизм песочницы недоступен. Сверка worktree сама по себе не видит записи за его пределами, чтения секретов и сетевых запросов. Проверки, которым нужна запись, пишут во временный каталог. Поведение песочницы claude подтверждается отдельным явным live-запуском; обычные тесты проверяют только передаваемые настройки.
