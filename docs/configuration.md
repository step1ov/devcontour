...
Shell ревьюера всегда исполняется в песочнице ОС: у codex — `--sandbox read-only`, у claude — sandbox Claude Code без сети, с запретом записи в проверяемый каталог и чтения каталогов учётных данных (`~/.ssh`, `~/.aws`, `~/.config/gh` и др.), без выхода из песочницы и с отказом запуска, если механизм песочницы недоступен. Сверка worktree сама по себе не видит записи за его пределами, чтения секретов и сетевых запросов. Проверки, которым нужна запись, пишут во временный каталог. Остальной домашний каталог остаётся доступным на чтение: там лежат проверяемый checkout и инструменты; полная изоляция секретов — задача отдельного исполнителя. Поведение песочницы claude подтверждается явным live-запуском `npm run test:live:review-sandbox` (вызывает модель); обычные тесты проверяют только передаваемые настройки.
