...
- чтение закрыто для каталогов контура (база, worktrees, артефакты других прогонов) и каталогов учётных данных (`~/.ssh`, `~/.aws`, `~/.config/gh`, `~/.npmrc` и др.); собственный worktree и закреплённые зависимости открыты;
