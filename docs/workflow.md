...
Агент ведёт таблицу требований и progress-документ, сохраняет решения и доказательства. Большое ТЗ выполняется этапами: один небольшой принятый сценарий не считается выполнением всего продукта.
...
Простое появление файла в `docs` не запускает фоновый AI-процесс. Нужен один стартовый запрос агенту. Web-сервер исполняет утверждённую очередь; он не заменяет ведущего агента для bootstrap, новых этапов и разбора сбоев. Если его сессия остановилась, поручите продолжить по `<workspace>/docs/harness-progress.md` и состоянию harness.
