...
4. Наблюдайте за картой продукта и досками, затем получите итоговый результат. Агент сообщает о существенных блокерах; обычные технические шаги выполняет самостоятельно.
...
flowchart TD
    ...
    J -->|Нет| F
    J -->|Да| K[Полнота фич и совместные тесты релиза]
    K --> L[Приёмка продуктового ChangeSet]
    L --> M[Итоговый отчёт и результат]
```
