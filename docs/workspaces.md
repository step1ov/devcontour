# Несколько репозиториев в отдельном workspace

Этот документ описывает `separate`. Для одного проекта или монорепозитория можно выбрать `embedded` и хранить оркестрацию внутри него: [выбор размещения](workspace-modes.md).
...
Перед началом ведущий агент уточняет режим размещения и абсолютный путь, если они ещё не выбраны или не сохранены. До ответа setup и выполнение не начинаются. Уже переданный путь используется без повторного вопроса.
...
# Панель уже работает после раннего start; второй сервер не запускается.
```
