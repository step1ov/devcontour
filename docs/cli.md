...
`COMMAND` и условные IDs заменяются фактическими значениями. `--workspace` разрешается в `<workspace>/.harness/local`; альтернативный `--data` указывает папку с `config.json` напрямую. Их нельзя передавать вместе. Для старой конфигурации без workspaceRoot используйте `--data`.
...
Предпочтительный интерфейс ведущего агента — CLI. HTTP нужен UI и локальным интеграциям. Сервер слушает loopback; публикация в интернет не поддерживается без отдельного слоя аутентификации. Для mutating-запросов обязательны `Content-Type: application/json` и `X-Harness-Request: 1`, Host/Origin проходят проверку.
