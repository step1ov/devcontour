...
| `doctor`           | опционально `--probe`                                                                               | Проверяет готовность; probe дополнительно вызывает каждый runtime в окружении прогона, запускает настроенный preflight и read-only forge-доступ |
...
| `edit-board`      | `--board ID [--title ...] [--description ...]`                     | Правит название и цель доски, пока её план не утверждён; после утверждения цель меняет корректировка                   |
| `context-sources` | —                                                                  | Показывает источники чужих библиотек: репозиторий, закреплённый коммит, лицензия, разрешённые пути                     |
| `context-import`  | `--source ID` [`--paths a/,b/`]                                    | Вносит Markdown источника на закреплённом коммите в `packs/context/vendor/<id>/` с лицензией и происхождением          |
...
| `reopen`           | `--task ID --reason ...`                        | Возвращает в черновик задачу без принятого результата: утверждение снимается, доску нужно провести через ревью плана заново                                                                 |
| `retry`            | `--task ID` [`--reset --reason ...`]            | Возвращает failed/cancelled задачу в допустимое состояние, сохраняя историю; действует maxAttempts. `--reset` обнуляет исчерпанный бюджет попыток — причина обязательна и попадает в журнал |
...
| `export`              | выбранный контекст                            | Выводит собранные state/events JSON; не полная копия: для неё — `backup`                                                                                   |
| `backup`              | `--out FILE`                                  | Согласованная копия SQLite без остановки сервера; компонентное хранение не поддерживается                                                                  |
| `restore`             | `--from FILE --to DIR`                        | Восстанавливает копию в пустой каталог, не трогая workspace: снимает активное владение, сверяет SHA принятых результатов с Git; код выхода 1, если SHA нет |
