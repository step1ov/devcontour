...
DevContour отделяет намерение, исполнение и доказательство. Текст «сделано» от модели — сообщение исполнителя. Завершённая задача — запись доменного сервиса, принятая после проверок. Это основа harness-подхода: качество обеспечивается внешним по отношению к модели циклом управления.
...
Workspace — каталог координации. Он не обязан содержать исходники компонентов: они могут лежать рядом. Компонент — корень одного Git-репозитория с устойчивым `repositoryId`. Пакеты монорепозитория пока не становятся отдельными компонентами автоматически.
...
Ведущий агент — активная сессия во внешнем агентском инструменте. В MVP нет встроенного постоянно работающего продуктового менеджера. Завершение этой сессии не делает сервер способным самостоятельно придумать следующий этап.

## Сущности

| Термин                   | Значение                                                                                            |
| ------------------------ | --------------------------------------------------------------------------------------------------- |
| Requirement / требование | Ожидаемое поведение из ТЗ; трассировка до задач ведётся в документах проекта                        |
