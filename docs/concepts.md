...
Workspace — логическая граница координации, а не обязательный дополнительный репозиторий. В `embedded` он совпадает с корнем единственного Git проекта или монорепозитория; в `separate` расположен отдельно от самостоятельных репозиториев. Режим и путь выбирает пользователь перед первым запуском. Приложения и внутренние пакеты монорепозитория описываются в продуктовой карте. [Выбор размещения](workspace-modes.md).
