...
- **заняты сейчас** — карточка на исполнителя: роль, фаза с порядком (пишет код → проверки → ревью → интеграция), runtime и модель, кто проверит независимо, время работы и номер попытки; свободные места показаны отдельно, поэтому видно и загрузку;
