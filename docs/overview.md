...
2. **Агент разбирается в проекте.** Изучает код и правила, готовит окружение, выбирает подходящий стек и проверки. Для интерфейса определяет референсы, макет и дизайн-токены. В `INTENT.md` фиксирует цель, приложения, фичи и истории порученного релиза. Явно отмечает, что входит в каждое приложение сейчас, что отложено и что не применяется.
