...
Сначала пользователь выбирает [режим размещения](workspace-modes.md) и путь. Для separate до setup запустите `npm run devcontour -- start --workspace-mode separate --workspace /absolute/product-workspace --port 0`; для embedded используйте `--workspace-mode embedded --workspace /absolute/product`. Передайте пользователю URL. Через операции `preparation_*` ведущий агент сохраняет и отправляет на рассмотрение продуктовую постановку, после её утверждения — архитектуру и стек с C1/C2, затем референсы, концепт и дизайн-систему. Пользователь подтверждает каждую версию отдельно в панели. Форматы и корректировки описаны в [продуктовом процессе](staged-workflow.md). Команды ниже выполняются только после этих согласований.

## 1. Подготовить продукт и среду

Положите непустое ТЗ в `/absolute/product/docs/spec.md`. В separate workspace находится вне продукта; в embedded это корень продукта. Установка инструмента в обоих случаях отдельная. Сначала проверьте существующие правила, Git и незакоммиченные изменения: setup сохраняет файлы, но не решает смысловые противоречия между инструкциями.
