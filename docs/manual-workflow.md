...
Этот маршрут показывает работу ведущего агента изнутри. Оператору достаточно ТЗ и пути workspace; ручное исполнение всех шагов не является обязательным процессом. Для начала без моделей используйте [demo](quickstart.md), для нескольких репозиториев — [workspace guide](workspaces.md).
...
## 3. Согласовать архитектуру и контракты
...
## 5. Запустить консоль и очередь

В первом терминале или управляемой сессии:
