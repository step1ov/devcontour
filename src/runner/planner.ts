...
export async function plan(
  ...
  const request = {
    ...
    prompt: [
      ...
      'When the specification has REQ-* sections, use requirements_snapshot through the lead agent and preserve exact id/source/text/digest bindings. Link each requirement to a configured test gate and an observable scenario; never invent a digest. Where the gate reports individual testcases, also name the testId that proves the scenario, exactly as the report spells it; acceptance verifies that this testcase ran and passed on the accepted commit.',
